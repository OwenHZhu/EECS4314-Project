...
from discussion_service.services import discussion_service
...
def test_like_thread_and_count(fake_supabase):
    table = discussion_service.DISCUSSION_THREAD_LIKES_TABLE

    result = discussion_service._like_thread(user_id="u1", thread_id="t1")

    assert result is True
    assert fake_supabase.table(table).calls[0][0] == "insert"

    fake_supabase.table(table).response = FakeResponse(data=[{"user_id": "u1"}], count=1)
...
def test_toggle_thread_like_uses_unlike_when_already_liked(fake_supabase):
    ...
    # simulate "already liked" so toggle takes the unlike branch
    fake_supabase.table(table).response = FakeResponse(data=[{"user_id": "u1"}], count=1)
    ...
    assert ("delete", None) in fake_supabase.table(table).calls